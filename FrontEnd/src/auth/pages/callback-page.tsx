...
export function CallbackPage() {
  ...
  const { saveAuth, setCurrentUser: setCurrentUser } = useAuth();
  ...
  useEffect(() => {
    ...
    if (errorParam) {
      setError(errorDescription || 'Σφάλμα κατά την πιστοποίηση');
      // After a delay, redirect to signin page with error params
      setTimeout(() => {
        navigate(
          `/auth/signin?error=${errorParam}&error_description=${encodeURIComponent(errorDescription || 'Σφάλμα κατά την πιστοποίηση')}`,
        );
      }, 1500);
      return;
    }
    ...
    const handleCallback = async () => {
      try {
        ...
        const authModel = {
          ...
          jwtExpire: String(data.session.expires_at || Date.now() + 3600000),
        };
      ...
      {error ? (
        <div className="space-y-4">
          <h2 className="text-xl font-semibold text-destructive">
           Σφάλμα κατά την πιστοποίηση
          </h2>
          <p className="text-muted-foreground">{error}</p>
          <p className="text-sm">Ανακατεύθυνση στη σελίδα σύνδεσης...</p>
        </div>
      ) : null}
    </div>
  );
